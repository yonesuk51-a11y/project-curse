...
(function(){
  ...
  function check(){
    ...
    return {name:'archiveIndex',patch:'5.35.0',ok:ids===expectedIds&&cards.every(card=>card.dataset.pcArchiveCategory)&&(!verdicts||verdictRows.length===verdicts.getSummary().total),records:cards.length,verdicts:verdictRows.length,issues:ids===expectedIds?[]:[{level:'error',code:'PUBLIC_INDEX_MISMATCH',message:ids}]};
  }
