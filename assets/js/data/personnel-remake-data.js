...
(function(root){
  ...
  // 2026-09-25 사용자 채택 — 남은 8명(데미안·리아·요나스·엘리제 발렌 A, 제이크 로웰 A, 나탈리아 볼코바 B, 아마미야 레이 B, 야나미 미즈호 B). 미즈호는 젊은 외모.
  records['alma-damian'].visual={src:'assets/resources/derived/alma-damian-portrait-concept-v1.png',className:'RECONSTRUCTED',label:'PORTRAIT RECONSTRUCTION / 인물 재구성',alt:'밤의 감사실 책상등 아래에서 둥근 안경을 쓴 곱슬머리의 데미안 발렌이 펼친 장부를 짚고 앉아 있다.',caption:'대조용 인물 재구성 스케치다. 인상착의 기록이 없어 외형을 새로 재구성했다. 장부의 내용과 실제 감사 사건은 확정하지 않는다.'};
  records['alma-bennett'].visual={src:'assets/resources/derived/alma-bennett-portrait-concept-v1.png',className:'RECONSTRUCTED',label:'PORTRAIT RECONSTRUCTION / 인물 재구성',alt:'사각 작업등이 켜진 진료소 천막에서 머리를 높게 묶은 리아 발렌이 의료 가방 옆에 서서 장갑을 끼고 있다.',caption:'대조용 인물 재구성 스케치다. 인상착의 기록이 없어 외형을 새로 재구성했다. 환자의 신원과 이 장소의 실제 치료 사건은 확정하지 않는다.'};
  records['alma-griffon'].visual={src:'assets/resources/derived/alma-griffon-portrait-concept-v1.png',className:'RECONSTRUCTED',label:'PORTRAIT RECONSTRUCTION / 인물 재구성',alt:'격리문 옆 정비 의자에 앉은 요나스 발렌이 헝클어진 갈색 머리 아래로 올려다보며 큰 스패너를 무릎에 놓고 있다.',caption:'대조용 인물 재구성 스케치다. 인상착의 기록이 없어 외형을 새로 재구성했다. 격리문의 개폐 상태와 수동 해제장치의 위치는 확정하지 않는다.'};
  records['alma-wade'].visual={src:'assets/resources/derived/alma-wade-portrait-concept-v1.png',className:'RECONSTRUCTED',label:'PORTRAIT RECONSTRUCTION / 인물 재구성',alt:'어두운 의료기록실에서 안경을 쓴 엘리제 발렌이 닫힌 검체함에 손을 얹고 무지 서류철을 들고 있다.',caption:'대조용 인물 재구성 스케치다. 인상착의 기록이 없어 외형을 새로 재구성했다. 검체의 내용과 원본 검체의 위치는 확정하지 않는다.'};
  records['jake'].visual={src:'assets/resources/derived/jake-portrait-concept-v1.png',className:'RECONSTRUCTED',label:'PORTRAIT RECONSTRUCTION / 인물 재구성',alt:'짧은 머리와 콧수염의 제이크 로웰이 어두운 비상구 문을 잡고 접힌 도면을 든 채 복도를 살핀다.',caption:'대조용 인물 재구성 스케치다. 인상착의 기록이 없어 외형을 새로 재구성했다. 2005년 호송대 매복 이후의 생사는 확정하지 않는다.'};
  records['natalia'].visual={src:'assets/resources/derived/natalia-portrait-concept-v1.png',className:'RECONSTRUCTED',label:'PORTRAIT RECONSTRUCTION / 인물 재구성',alt:'짧은 적갈색 머리와 둥근 안경의 나탈리아 볼코바가 녹음기를 안고 기록 대기실의 운반대에 서류철을 내려놓는다.',caption:'대조용 인물 재구성 스케치다. 인상착의 기록이 없어 외형을 새로 재구성했다. 특정 회의와 정보원의 신원은 확정하지 않는다.'};
  records['reiki'].visual={src:'assets/resources/derived/reiki-portrait-concept-v1.png',className:'RECONSTRUCTED',label:'PORTRAIT RECONSTRUCTION / 인물 재구성',alt:'짧은 갈색 머리의 아마미야 레이가 창고 선반 사이 출입문 앞에 웅크려 손전등으로 바닥을 비춘다.',caption:'대조용 인물 재구성 스케치다. 인상착의 기록이 없어 외형을 새로 재구성했다. 귀환한 신체의 정체와 사망 처리 뒤의 생존 여부는 확정하지 않는다.'};
  records['mizumi-yanami'].visual={src:'assets/resources/derived/mizumi-yanami-portrait-concept-v1.png',className:'RECONSTRUCTED',label:'PORTRAIT RECONSTRUCTION / 인물 재구성',alt:'라벤더색 불이 켜진 수몰 통로의 턱에 짧은 짙은 머리의 야나미 미즈호가 긴 코트와 검붉은 목도리 차림으로 앉아 있고, 발치로 검붉은 피가 흐른다.',caption:'대조용 인물 재구성 스케치다. 겉모습이 기록된 나이(44세)보다 훨씬 젊다. 젊음을 유지하는 힘의 정체와 대가, 피의 강 결계의 범위는 확정하지 않는다.'};
