...
(function(root){
  ...
  const publicAnchors=[
    {
      ...
      boundary:'제6계측계획은 이 공개 사업의 비밀 부문으로 확인되지 않았다. 공개된 병렬처리 연구와 동시대 산업 기반을 차용한 별도 비인가 계측 조직으로 분류한다.',
    ...
    },
    {
      ...
      boundary:'JID-87은 공개된 TRON 규격과 일치하지 않는다. 동시대의 개방형 임베디드 제어 흐름을 초상 감시 장치에 적용한 비공개 내부 규격으로 분류한다.',
  ...
  const records=[
    {
      ...
      fragments:[
        ...
        {label:'조달 주석',kind:'annotation',text:'초기 장비는 시판 계측기와 국산 프로세서를 조합했다. 아마리온 또는 F.H.C의 공간 장치가 핵심부에 사용됐다는 증거는 없다. 공급업체 코드 두 건은 회사 승계대장과 형식이 비슷하다. 원본 송장은 남아 있지 않다.'}
      ]
    },
    {
      ...
      basis:'송신 원문은 같았다. 세 시험기의 반환 위상과 지연 배열은 경로별로 반복 재현됐다.',sourceState:'시험 테이프 3본 / 원 송신장치 폐기',
      fragments:[
        ...
        {label:'현장 결론',kind:'analysis',text:'변조된 메시지는 원문을 흉내 낼 수 있다. 경로 전체의 지연과 반환 위상은 동시에 복제하지 못했다. 시험반은 이 배열을 광반환 지문이라 불렀다. 무엇이 C 회선을 통과했는지는 보고하지 않았다.'},
    ...
    },
    {
      ...
      summary:'폐쇄형 규격이 채택됐다. 서로 다른 제조사의 소형 장치가 같은 시차·반환 지문 형식으로 보고하도록 했다.',
      ...
      fragments:[
        ...
        {label:'후대 감사',kind:'annotation',text:'금지 조항은 모든 지역에서 지켜지지 않았다. 일부 공안 시험소는 출입기록과 계측값을 결합했다. 그 목록은 1993년 S.I.D 초기 도시 감시망으로 넘어갔다.'}
      ]
    },
    {
      ...
      fragments:[
        {label:'실증 결과',kind:'analysis',text:'중앙 회선이 11분 동안 끊겼다. 각 노드는 지역 시계와 인접 장치의 확인값을 보존했다. 복구 뒤 64개 기록을 합쳤다. 전력 저하, 신호기 정지와 비정합 파형의 순서를 초 단위로 재구성할 수 있었다.'},
    ...
    },
    {
      ...
      basis:'해산명령과 지자체 자산대장의 장치 수는 다르다. 중앙 인력 해제일과 규격 이관처는 일치한다.',sourceState:'해산명령 / 지역 자산대장 수량 상충',
      fragments:[
        ...
        {label:'수량 상충',kind:'analysis',text:'해산위원회는 운용 노드 418개를 기록했다. 지역 자산대장에서는 603개가 확인된다. 차이 가운데 일부는 공장 자동화와 가정용 실증설비로 재분류됐다. 37개는 주소 자체가 검게 지워져 있다.'},
        {label:'1993년 이후',kind:'annotation',text:'U.A.C 공개 설립 뒤 일본은 원시 로그가 아니라 인터페이스 규격과 반환 지문 비교법만 제공했다. 이 기술은 훗날 C.I. 스캐너와 북부전선의 복제 신호 분리 절차에 반영됐다. 두 장비가 일본제 장치의 단순 개명판은 아니다.'}
      ]
    }
  ];
  ...
  root.ProjectCurseJapanTechnology=freeze({
    ...
    socialOutcomes:[
      ...
      {label:'경제 기록 대조',text:'버블기의 설비투자는 확산을 가속했지만, 버블의 발생 원인과 붕괴 시점은 공개 경제 기록과 일치한다.'},
