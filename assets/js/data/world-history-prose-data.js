...
(function(root){
  ...
  const records={
    ...
    '1986-02-01-immortality':{
      ...
      counterRecord:{
        ...
        fragments:[
          ...
          {label:'회수 경위',kind:'annotation',text:'이 문서는 1991년 압수된 문서철에서 나왔다. 필사본이며 원본은 회수되지 않았다. 작성 집단이 혈교의 어느 계보인지, 부름이 회수 작전보다 앞섰는지 뒤였는지는 확인되지 않았다. 같은 밤을 적은 현장 보고와 시각 표기 방식이 달라 두 기록의 순서는 맞추지 못했다.'}
        ],
    ...
    '2012-11-19-great-black-forest-survey':{
      ...
      fragments:[
        ...
        {label:'관측소 조치',kind:'document',text:'고정 경계선 작성은 중단됐다. 지도 표제는 GREAT BLACK FOREST로 고정하고, 행정구역 대신 통과가 확인된 회랑, 불이 켜진 성채와 마지막 교신 지점을 표시한다. SOUTHERN DEAD ZONE은 구문서 인용에만 남긴다.'}
      ]
    },
    ...
    '2016-02-21-blood-cult-atlantic-schism':{
      ...
      fragments:[
        ...
        {label:'분석관 주석',kind:'analysis',text:'양측은 같은 의식어를 사용했지만 이후 보급로와 인명 명부를 공유하지 않았다. 교리 분쟁만으로는 이 결별을 설명하기 어렵다. 데드존 지부는 검문소·중립 마을과의 교환 없이는 회랑을 유지할 수 없었다.'}
      ]
    },
    ...
    '2034-04-22-inland-beacon-31':{
      ...
      fragments:[
        ...
        {label:'파형 대조',kind:'finding',text:'호출부호 1–8은 2007년 이전 구조대 대장과 일치. 9–31은 등록 이력 없음. 반복마다 음성 내용은 같고 경과 날짜만 하루씩 앞선다. 압축·접합 흔적은 검출되지 않았다.'},
    ...
    '2036-12-12-central-callsign-loss':{
      ...
      fragments:[
        ...
        {label:'감청 판정',kind:'analysis',text:'공통 호출명으로 세 파벌의 동시 행동을 설명할 수 없게 됐다. 타락교 문구가 남부 보급명령과 F.H.C 잔존시설 경고문에 함께 남았으나, 발신 지문은 서로 일치하지 않는다.'},
    ...
    '2042-10-31-three-night-silence':{
      ...
      fragments:[
        ...
        {label:'합동실 유보',kind:'annotation',text:'동시 무응답과 교차 호명은 보존한다. 장거리 전송·시간 중첩·제3자 모방은 구분하지 못했다. 지도에는 관측점만 남기며 두 권역 사이 경로를 만들지 않는다.'}
      ]
    }
  };
  ...
  const recordLimits={
    ...
    '1986-02-01-immortality':'회수 순서와 현장 기록은 보존됐지만 호수의 기원과 실종 인원의 최종 상태는 확인되지 않았다.',
