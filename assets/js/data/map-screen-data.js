...
'use strict';
  const canonBoundary=Object.freeze({
    ...
    lineageGuard:'어느 선택도 남방 혈맹을 혈교의 확정 후계로 만들지 않으며, 데드존 혈교를 혈맹 지휘 아래 재편입시키지 않는다. 같은 표식·의식·일시적 교신은 동일한 지휘 계보의 증거가 아니다.'
  });
  const decisions={
    execute:{
      ...
      unresolved:'처형 명령의 원 발신자와 지휘관의 실제 충성은 확인되지 않았다. 혈맹 강경파가 전후 지휘권을 장악했다는 결론도 승인되지 않았다.',
      consequence:'현재 지도 사본에서 지휘망 붕괴와 소환진 재활성 위험을 추적한다. 혈맹 전체의 권력 승계는 미확정이다.',
    ...
    detain:{
      ...
      unresolved:'지휘관이 혈맹 명령에 불복한 것인지, 더 깊은 침투를 위해 협조한 것인지는 판단할 수 없다.',
    ...
    cooperate:{
      ...
      unresolved:'혈맹 강경파의 공개 적대, 지휘관의 장기 충성, 데드존 혈교의 공식 가담 여부는 확정되지 않았다.',
      consequence:'현재 지도 사본에 공동 차단선과 비인가 교신을 표시한다. 데드존 혈교가 혈맹 또는 U.A.C 지휘에 편입된 것은 아니다.',
    ...
    const theaters=[
      ...
      {
        id:'south',number:'04',eyebrow:'OPERATION BROKEN CROWN',title:'남방 쿠데타',
        status:'CRITICAL / PARTIAL',confidence:'부분 감청',tone:'coup',
        summary:'남방 해안의 집단 소환과 성위대 침투가 한 작전으로 수렴한다. 지휘 계통은 이미 오염됐다.',
...
root.ProjectCurseMapScreenData=Object.freeze({
 ...
 copy:{
 timeline:'고른 해까지 날짜가 확인된 기록을 표시합니다. 연도를 알 수 없는 기록은 점선으로 남습니다.',
 timelineUnknown:'연도 알 수 없음',
 mapControls:'휠·두 손가락으로 확대, 끌어서 이동. 지도에 초점을 두고 + / − / 0 / 방향키로도 조작합니다.',
 signalBoundary:'짧은 선은 각 관측 지점의 신호입니다. 지점 사이의 통로를 뜻하지 않습니다.',
 noPhoto:'연결된 현장 사진 없음',
 navigation:'표시 좌표는 길을 찾는 데 쓰지 않는다. 신뢰도와 서로 맞지 않는 기록을 함께 확인한다.',
 withheld:'위치 공개 보류는 사건이 없었다는 뜻이 아니다. 지도에 표시할 지점이 승인되지 않아 위치로 이동할 수 없다.',
 index:'지도 지점, 작전, 독립 관측, 위치 공개가 보류된 기록을 함께 찾는다.',
 intro:'지도는 세계를 설명하지 않는다. 남아 있는 신호와 돌아오지 못한 사람들의 경로만 표시한다.',
 archive:'직접 확인한 결과만 열린다. 최종 판정 순간의 선택과 측정값은 원본 기록과 분리한 판정 기록으로 보존된다.',
