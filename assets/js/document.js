(function(){
  ...
  document.querySelectorAll('a,button').forEach(el=>{ if(!el.matches('[data-sound-toggle], .page-btn')) el.addEventListener('click',()=>{play(click,.20);}); });
  document.querySelectorAll('.back-button').forEach(el=>el.addEventListener('click',e=>{ if(soundOn){e.preventDefault(); play(openFx,.28); setTimeout(()=>{window.location.href=el.href},240);} }));
  ...
  function initPageViewer(){
    ...
    toolbar.innerHTML=`
      <div class="page-readout">
        <span class="page-readout-label">SLIDE DOCUMENT FEED</span>
      ...
      <div class="page-hint">A/D 또는 ←/→ 키로 넘김</div>
    `;
    ...
    function render(index, direction){
      ...
      if(direction!==0){
        play(slideFx,.46);
        setTimeout(()=>play(pageGlitch,.22),120);
