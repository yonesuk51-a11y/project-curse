...
check('version:manifest',context.window.ProjectCurseStructure?.version==='5.32.0',context.window.ProjectCurseStructure?.version);
check('version:archive',context.window.ProjectCurseArchive?.version==='5.32.0',context.window.ProjectCurseArchive?.version);
check('version:documents',context.window.ProjectCurseArchiveDocuments?.version==='5.32.0',context.window.ProjectCurseArchiveDocuments?.version);
...
check('shell:adaptive-quality-policy',index.includes('assets/js/core/quality-policy.js?v=5.32.0')&&index.indexOf('quality-policy.js')<index.indexOf('adaptive-media.js')&&qualityPolicyRuntime.includes('ProjectCurseQuality=Object.freeze')&&qualityPolicyCss.includes('.pc-quality-diagnostics'));
...
check('document:root-loaded',index.includes('assets/css/archive-document.css?v=5.32.0')&&index.includes('assets/js/pages/archive-document.js?v=5.32.0')&&index.includes('assets/js/data/field-dossier-data.js?v=5.32.0'));
check('evidence:root-loaded',index.includes('assets/css/visual-evidence.css?v=5.32.0')&&index.includes('assets/js/data/visual-evidence-data.js?v=5.32.0')&&index.indexOf('visual-evidence-data.js')<index.indexOf('field-dossier-data.js'));
...
check('media:root-load-order',index.includes('assets/css/adaptive-media.css?v=5.32.0')&&index.indexOf('media-manifest.js')<index.indexOf('adaptive-media.js')&&index.indexOf('adaptive-media.js')<index.indexOf('archive-document.js'));
...
check('media:transition-warmup',read('assets/js/core/transition-controller.js').includes('ProjectCurseMedia.prepareRoute')&&read('assets/js/core/transition-controller.js').includes('VISUAL CHANNEL ACQUISITION'));
...
check('audio:semantic-profile-manifest',audioManifest.includes("version:'2.1.0'")&&audioManifest.includes("'great-black-forest'")&&audioManifest.includes("'dead-zone'")&&audioManifest.includes("'scenario.arm'")&&audioManifest.includes("'recovery-scenario'"));
...
check('operation:root-loaded',index.includes('assets/js/core/operation-state.js?v=5.32.0')&&index.indexOf('assets/js/core/operation-state.js')>index.indexOf('assets/js/core/audio-controller.js')&&index.indexOf('assets/js/core/operation-state.js')<index.indexOf('assets/js/core/record-cinematic-runtime.js'));
const drilldowns=context.window.ProjectCurseRegionalDrilldown?.districts||[];
check('map:drilldown-root-loaded',index.includes('assets/js/data/regional-drilldown-data.js?v=5.32.0')&&index.indexOf('assets/js/data/regional-drilldown-data.js')<index.indexOf('assets/js/data/map-room-data.js'));
...
check('mobile:cinematic-override-loaded-last',index.includes('assets/css/record-cinematic-mobile.css?v=5.32.0')&&index.indexOf('assets/css/record-cinematic-mobile.css')>index.indexOf('assets/css/app-shell.css'));
...
check('immortality:canonical-storyboard-loaded',index.includes('assets/js/data/immortality-storyboard.js?v=5.32.0')&&immortalityPages.length===24,immortalityPages.length);
...
].every(file=>index.includes(`${file}?v=5.32.0`)));
check('cache:cinematic-busters',index.includes('assets/css/record-cinematic.css?v=5.32.0')&&index.includes('assets/js/core/record-cinematic-runtime.js?v=5.32.0'));
check('cache:mobile-cinematic-buster',index.includes('assets/css/record-cinematic-mobile.css?v=5.32.0'));
