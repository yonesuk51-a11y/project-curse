...
for(const file of [
  'assets/js/data/build-info.js','assets/js/data/site-manifest.js','assets/js/data/channel-identity-data.js','assets/js/data/personnel-profile-data.js','assets/js/data/personnel-remake-data.js','assets/js/data/personnel-data.js','assets/js/data/archive-registry.js','assets/js/data/archive-document-data.js','assets/js/data/visual-evidence-data.js','assets/js/data/media-manifest.js','assets/js/data/media-provenance-data.js','assets/js/data/field-dossier-data.js','assets/js/data/regional-drilldown-data.js','assets/js/data/map-room-data.js',
...
check('personnel:independent-dossiers',context.window.ProjectCursePersonnel?.records?.length===56&&context.window.ProjectCursePersonnel?.stats?.profiled===56&&!context.window.ProjectCursePersonnel?.byId?.frey&&context.window.ProjectCursePersonnel?.editorialRule?.includes('개편 정본명')&&personnelRuntime.includes('ProjectCursePersonnelRuntime=Object.freeze'));
...
check('history:four-new-reconstruction-visuals',['2005-01-21-ash-crew','2036-12-12-central-callsign-loss','2038-06-29-sixth-northern-line','2042-10-31-three-night-silence'].every(id=>historyProse.includes(`'${id}':{`)&&historyProse.includes(`assets/resources/derived/${id}_001_reconstructed.png`)));
...
check('media:manifest-twenty-eight-sources',context.window.ProjectCurseMediaManifest?.version==='1.0.0'&&Object.keys(context.window.ProjectCurseMediaManifest?.assets||{}).length===28);
check('media:regional-brief-responsive',['assets/resources/derived/north-sea-blood-lake-blockade-reconstruction-concept-v1.png','assets/resources/derived/dead-zone-silent-interior-map-termination-concept-v1.png'].every(source=>context.window.ProjectCurseMediaManifest?.assets?.[source]?.variants?.map(variant=>variant.width).join('|')==='480|960'));
check('media:world-history-responsive',['assets/resources/derived/2005-01-21-ash-crew_001_reconstructed.png','assets/resources/derived/2036-12-12-central-callsign-loss_001_reconstructed.png','assets/resources/derived/2038-06-29-sixth-northern-line_001_reconstructed.png','assets/resources/derived/2042-10-31-three-night-silence_001_reconstructed.png'].every(source=>context.window.ProjectCurseMediaManifest?.assets?.[source]?.variants?.map(variant=>variant.width).join('|')==='480|960'));
check('media:provenance-all-assets',context.window.ProjectCurseMediaProvenance?.stats?.registered===204&&context.window.ProjectCurseMediaProvenance?.assets?.length===204);
check('media:provenance-honest-review',context.window.ProjectCurseMediaProvenance?.stats?.review===150&&context.window.ProjectCurseMediaProvenance?.stats?.managed===54&&context.window.ProjectCurseMediaProvenance?.stats?.referenceExposure===0);
...
check('media:world-history-runtime-binding',read('assets/js/pages/world-history.js').includes('window.ProjectCurseMedia.apply')&&!read('assets/js/pages/world-history.js').includes('root.ProjectCurseMedia'));
