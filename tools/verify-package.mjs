...
add('regional-drilldown-owned',structureData?.owners?.regionalDrilldownData==='assets/js/data/regional-drilldown-data.js'&&drilldowns.length===7&&context.window.ProjectCurseMapRoom?.drilldowns===drilldowns,drilldowns.length);
add('regional-drilldown-coverage',drilldowns.filter(detail=>detail.region==='eastasia').length===1&&drilldowns.filter(detail=>detail.region==='southamerica').length===3&&drilldowns.filter(detail=>detail.region==='northamerica').length===3&&drilldowns.reduce((total,detail)=>total+detail.sites.length,0)>=47);
...
add('regional-twenty-two-owned-routes',detailRoutes.length===22&&detailRoutes.every(({route})=>route.siteIds?.length>=2&&route.risk&&route.signal&&route.rule),detailRoutes.length);
