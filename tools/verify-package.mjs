...
add('media-provenance-all-192-assets',provenanceAssets.length===192&&repositoryMedia.join('|')===provenancePaths.join('|'),`${provenanceAssets.length} registered / ${repositoryMedia.length} files`);
...
add('media-provenance-kind-counts',mediaProvenance?.stats?.byKind?.image===162&&mediaProvenance?.stats?.byKind?.audio===23&&mediaProvenance?.stats?.byKind?.video===7);
add('media-provenance-honest-review',provenanceAssets.filter(asset=>asset.kind==='audio'||asset.kind==='video').every(asset=>asset.release==='LICENSE_REVIEW')&&mediaProvenance?.stats?.review===150&&mediaProvenance?.stats?.managed===42);
...
add('media-provenance-delivery-lineage',provenanceAssets.filter(asset=>asset.provenance==='DELIVERY_DERIVATIVE').length===48&&provenanceAssets.filter(asset=>asset.provenance==='DELIVERY_DERIVATIVE').every(asset=>asset.derivedFrom&&provenancePaths.includes(asset.derivedFrom)));
...
add('adaptive-media-twenty-four-sources',mediaManifest?.version==='1.0.0'&&responsiveAssets.length===24&&responsiveVariants.length===48,`${responsiveAssets.length} sources / ${responsiveVariants.length} variants`);
...
add('history-visual-responsive-runtime',worldHistory.includes('image.dataset.pcSource=record.visual.src')&&worldHistory.includes('ProjectCurseMedia.apply')&&worldHistory.includes("sizes:'(max-width: 760px) 94vw, 960px'"));
...
const publicConceptAssets=['project-curse-world-keyart-concept-v1.png','great-black-forest-unlit-fortress-bell-concept-v1.png','checkpoint-07-five-thermal-concept-v1.png','broken-crown-erased-commander-concept-v1.png','first-apostle-three-traces-reconstruction-concept-v1.png','joint-response-unit-unlisted-eleventh-group-concept-v1.png','nhc-young-soldiers-forward-base-group-photo-concept-v1.png','northern-front-duplicate-signal-reconstruction-concept-v1.png','north-sea-blood-lake-blockade-reconstruction-concept-v1.png','dead-zone-silent-interior-map-termination-concept-v1.png'];
const publicConceptSources=[index,fieldDossierData,factionAnalysisSource,worldHistoryProseSource,visualEvidenceData].join('\n');
add('ten-concept-images-selectively-integrated',publicConceptAssets.every(name=>publicConceptSources.includes(name))&&!publicConceptSources.includes('project-curse-world-keyart-concept-v2.png'));
