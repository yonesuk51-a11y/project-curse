...
add('media-provenance-all-188-assets',provenanceAssets.length===188&&repositoryMedia.join('|')===provenancePaths.join('|'),`${provenanceAssets.length} registered / ${repositoryMedia.length} files`);
...
add('media-provenance-kind-counts',mediaProvenance?.stats?.byKind?.image===158&&mediaProvenance?.stats?.byKind?.audio===23&&mediaProvenance?.stats?.byKind?.video===7);
add('media-provenance-honest-review',provenanceAssets.filter(asset=>asset.kind==='audio'||asset.kind==='video').every(asset=>asset.release==='LICENSE_REVIEW')&&mediaProvenance?.stats?.review===150&&mediaProvenance?.stats?.managed===38);
...
add('media-provenance-delivery-lineage',provenanceAssets.filter(asset=>asset.provenance==='DELIVERY_DERIVATIVE').length===44&&provenanceAssets.filter(asset=>asset.provenance==='DELIVERY_DERIVATIVE').every(asset=>asset.derivedFrom&&provenancePaths.includes(asset.derivedFrom)));
...
add('adaptive-media-twenty-two-sources',mediaManifest?.version==='1.0.0'&&responsiveAssets.length===22&&responsiveVariants.length===44,`${responsiveAssets.length} sources / ${responsiveVariants.length} variants`);
