...
add('channel-identity-six-channels',channelIdentityData?.channels?.length===6&&channelIdentityData.channels.map(channel=>channel.id).join('|')==='terminal-home|map-room|history|faction-info|archive-entry|media-audit');
add('channel-identity-distinct-themes',new Set(channelIdentityData?.channels?.map(channel=>channel.theme)).size===6&&['command','cartography','chronology','intelligence','archive','clearance'].every(theme=>channelIdentityCss.includes(`[data-pc-channel-theme="${theme}"]`)||channelIdentityCss.includes(`[data-channel-theme="${theme}"]`)));
...
add('telemetry-live-six-channels',['terminal-home','map-room','history','faction-info','archive-entry','media-audit'].every(id=>performanceTelemetry.includes(`id==='${id}'`))&&channelIdentityRuntime.includes('ProjectCurseTelemetry?.getChannelStatus'));
...
add('shell-quick-nav-five-links',(index.match(/<a[^>]+data-uac-route="(?:map-room|history|faction-info|archive-entry|media-audit)"[^>]*>/g)||[]).length>=5&&appShell.includes('closeQuickNav'));
...
add('current-six-screen-manifest',structureData?.screens?.map(screen=>screen.id).join('|')==='terminal-home|map-room|history|faction-info|archive-entry|media-audit');
