...
add('regional-recovered-briefing-network',recoveredBriefings.length===6&&new Set(recoveredBriefings.map(detail=>detail.visual.src)).size===6&&recoveredBriefings.every(detail=>existsSync(path(detail.visual.src))&&!detail.visual.src.includes('candidate')&&detail.visual.siteIds?.every(id=>detail.sites.some(site=>site.id===id))&&detail.signalBrief.siteIds?.every(id=>detail.sites.some(site=>site.id===id))&&detail.signalBrief.lanes?.length===2&&detail.signalBrief.checks?.length===3&&detail.signalBrief.log?.length===3),recoveredBriefings.map(detail=>detail.id).join('|'));
...
add('media-provenance-kind-counts',mediaProvenance?.stats?.byKind?.image===156&&mediaProvenance?.stats?.byKind?.audio===23&&mediaProvenance?.stats?.byKind?.video===7);
add('media-provenance-honest-review',provenanceAssets.filter(asset=>asset.kind==='audio'||asset.kind==='video').every(asset=>asset.release==='LICENSE_REVIEW')&&mediaProvenance?.stats?.review===150&&mediaProvenance?.stats?.managed===36);
