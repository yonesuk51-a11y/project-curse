...
add('channel-identity-seven-channels',channelIdentityData?.channels?.length===7&&channelIdentityData.channels.map(channel=>channel.id).join('|')==='terminal-home|map-room|history|faction-info|archive-entry|media-audit|personnel');
...
add('shell-quick-nav-seven-channels',(index.match(/<a[^>]+data-uac-route="(?:map-room|history|faction-info|archive-entry|media-audit|personnel)"[^>]*>/g)||[]).length>=6&&appShell.includes('closeQuickNav'));
...
add('current-seven-screen-manifest',structureData?.screens?.map(screen=>screen.id).join('|')==='terminal-home|map-room|history|faction-info|archive-entry|media-audit|personnel');
...
add('personnel-registry-schema',personnelData?.version===VERSION&&personnelData?.schema==='project-curse-personnel-v1'&&personnelData.records.length===57&&personnelData.groups.length===10,`${personnelData?.records?.length||0} records / ${personnelData?.groups?.length||0} groups`);
add('personnel-registry-unique-ids',new Set(personnelData?.records?.map(record=>record.id)).size===57);
add('personnel-world-first-boundary',personnelData?.editorialRule?.includes('세계관 우선')&&personnelData.byId?.frey?.overview?.includes('중심인물로 지정되지는 않으며')&&personnelRuntime.includes('연결 수는 중요도나 주인공 지위를 뜻하지 않는다'));
...
add('personnel-responsive-presentation',personnelCss.includes('@media(max-width:900px)')&&personnelCss.includes('@media(max-width:600px)')&&personnelCss.includes('@media(prefers-reduced-motion:reduce)'));
...
add('personnel-manifest-ownership',structureData?.owners?.personnelData==='assets/js/data/personnel-data.js'&&structureData?.owners?.personnelRuntime==='assets/js/pages/personnel-archive.js'&&structureData?.owners?.personnelCSS==='assets/css/personnel-archive.css');
