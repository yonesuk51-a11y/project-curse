...
add('shell-quick-nav-five-lore-only',(quickNav.match(/data-uac-route=/g)||[]).length===5&&!quickNav.includes('media-audit')&&['map-room','history','faction-info','archive-entry','personnel'].every(route=>quickNav.includes(`data-uac-route="${route}"`))&&channelIdentityRuntime.includes("channel.navTier==='utility'")&&appShell.includes('closeQuickNav'));
...
add('home-utility-register-separated',index.includes('class="pc-terminal-utility-drawer"')&&index.includes('단말 유지보수 채널')&&index.includes('data-pc-preferences-open')&&count(index,'<b>미디어 감사</b>')===1&&!quickNav.includes('media-audit'));
...
add('current-six-lore-one-utility-manifest',structureData?.screens?.map(screen=>screen.id).join('|')==='terminal-home|map-room|history|faction-info|archive-entry|personnel|media-audit'&&structureData.screens.find(screen=>screen.id==='media-audit')?.navTier==='utility');
...
add('media-provenance-unselected-candidates-excluded',[...unselectedCandidateMedia].every(relative=>existsSync(path(relative))&&!provenancePaths.includes(relative)&&!index.includes(relative.split('/').at(-1))));
...
add('personnel-2006-snapshot-display',personnelRuntime.includes('U.A.C PERSONNEL REGISTER / 2006 SNAPSHOT')&&personnelRuntime.includes("active:'2006년 활동 확인'")&&personnelRuntime.includes("deceased:'2006년 사망 기재'")&&personnelRuntime.includes("unknown:'2006년 이후 미확인'")&&personnelRuntime.includes('활동 표시는 2042년 현재의 생존이나 재직을 뜻하지 않는다'));
add('personnel-summary-before-selection',personnelRuntime.includes('function summaryDetailMarkup')&&personnelRuntime.includes('선택 전 명부 판독')&&personnelRuntime.includes('STATUS TRIAGE')&&personnelCss.includes('.pc-personnel-summary'));
...
add('personnel-dossier-presentation',personnelRuntime.includes('function identityMarkup')&&personnelRuntime.includes('function backgroundMarkup')&&personnelRuntime.includes('function psychologyMarkup')&&personnelRuntime.includes('function fieldNotesMarkup')&&personnelRuntime.includes('record.sourceName'));
