...
const mediaClearanceRuntime=read('assets/js/pages/media-clearance.js');
const mediaClearanceCss=read('assets/css/media-clearance.css');
...
const mapSignalIndexSource=read('assets/js/data/map-signal-index-data.js');
...
vm.runInContext(mapSignalIndexSource,context,{filename:'map-signal-index-data.js'});
...
add('channel-identity-distinct-themes',new Set(channelIdentityData?.channels?.map(channel=>channel.theme)).size===6&&['command','cartography','chronology','intelligence','archive','personnel'].every(theme=>channelIdentityCss.includes(`[data-pc-channel-theme="${theme}"]`)||channelIdentityCss.includes(`[data-channel-theme="${theme}"]`)));
...
add('telemetry-live-seven-channels',['terminal-home','map-room','history','faction-info','archive-entry','media-audit','personnel'].every(id=>performanceTelemetry.includes(`id==='${id}'`))&&channelIdentityRuntime.includes('ProjectCurseTelemetry?.getChannelStatus'));
...
const quickNav=index.slice(index.indexOf('class="uac-shell-quick"'),index.indexOf('</nav>',index.indexOf('class="uac-shell-quick"')));
add('shell-quick-nav-five-lore-only',(quickNav.match(/data-uac-route=/g)||[]).length===5&&!quickNav.includes('media-audit')&&['map-room','history','faction-info','archive-entry','personnel'].every(route=>quickNav.includes(`data-uac-route="${route}"`))&&!channelIdentityRuntime.includes('도구')&&appShell.includes('closeQuickNav'));
...
add('home-three-incident-entry-branches',count(index,'data-story-phase=')===3&&index.includes('data-story-phase="exposure"')&&index.includes('data-story-phase="collapse"')&&index.includes('data-story-phase="current"')&&index.includes('data-uac-history-record="1986-02-01-immortality"')&&index.includes('data-uac-history-record="2042-10-31-three-night-silence"'));
add('home-reader-surface-no-maintenance-tools',!index.includes('class="pc-terminal-utility-drawer"')&&!index.includes('단말 유지보수 채널')&&!index.includes('data-pc-preferences-open')&&!index.includes('<b>미디어 감사</b>')&&!quickNav.includes('media-audit'));
...
add('operation-central-record-boundary',operationState.includes('CENTRAL RECORD UNCHANGED')&&operationState.includes('fixedFacts:Object.freeze([')&&operationState.includes('pendingFacts:Object.freeze([')&&operationState.includes("lineageGuard:'어느 선택도"));
...
add('operation-document-boundary-ui',read('assets/js/pages/archive-document.js').includes('archive-scenario-canon-boundary')&&read('assets/js/pages/archive-document.js').includes('archive-scenario-report-layers')&&read('assets/js/pages/archive-document.js').includes('중앙 기록 반영')&&read('assets/css/archive-document.css').includes('.archive-scenario-lineage-guard'));
...
add('operation-field-map-and-home-labels',mapRoomRuntime.includes('FIELD OPERATION COPY')&&mapRoomRuntime.includes('CENTRAL RECORD UNCHANGED')&&mapRoomRuntime.includes('pc-op-canon-boundary')&&terminalHomeRuntime.includes('LOCAL VERDICT SAVED')&&terminalHomeRuntime.includes('FIELD COPY / CENTRAL RECORD UNCHANGED'));
...
add('pilgrimage-field-verdict-boundary',pilgrimageRuntime.includes('FIELD VERDICT / CENTRAL ARCHIVE UNAPPROVED')&&pilgrimageRuntime.includes('pc-pilgrimage-feedback-boundary')&&pilgrimageRuntime.includes('current.canonBoundary')&&pilgrimageCss.includes('.pc-pilgrimage-ending-boundary'));
...
add('verdict-terminal-record-boundary',verdictState.includes('단말 보관 판정·중앙 기록 미승인')&&verdictState.includes("code:'FIELD VERDICT'")&&verdictState.includes('limit:scenario.canonBoundary')&&verdictState.includes('단말 관제 결과'));
...
add('initial-route-and-detail-safe',index.includes('pc5152ca1-terminal-home active')&&appShell.includes('const initialLocation=readLocation()')&&appShell.includes('const preserveInitialDetail=Boolean(initialLocation.personnelId||initialLocation.mapRoomPath.length)')&&appShell.includes("commitRoute(initialRoute,initialRoute,preserveInitialDetail?'none':'replace')"));
...
add('loading-modes-and-readable-timing',loadingRuntime.includes('duration:8200')&&loadingRuntime.includes('duration:1200')&&loadingRuntime.includes('duration:950')&&loadingRuntime.includes('duration:120')&&loadingRuntime.includes('MIN_VISIBLE_MS=900')&&loadingRuntime.includes('2600')&&loadingRuntime.includes('FINAL ACCESS HOLD')&&loadingRuntime.includes("classList.add('is-authorized')")&&loadingRuntime.includes('SESSION_KEY=()=>')&&loadingRuntime.includes("get('boot')")&&index.includes('data-boot-gate'));
...
const registeredAudioEvents=new Set(Object.keys(context.window.ProjectCurseAudioManifest?.events||{}));
const unknownAudioCalls=[];
fileTree(path('assets/js/')).filter(relative=>relative.endsWith('.js')).forEach(relative=>{
  const source=read(relative);
  for(const match of source.matchAll(/ProjectCurseAudioControl\?\.play\?\.\(\s*['"]([^'"]+)['"]/g)){
    if(!registeredAudioEvents.has(match[1])) unknownAudioCalls.push(`${relative}:${match[1]}`);
  }
});
add('semantic-audio-call-registry',unknownAudioCalls.length===0,unknownAudioCalls.join('|'));
...
add('northern-brief-semantic-audio',context.window.ProjectCurseAudioManifest?.events?.['map.brief']?.cue==='scan'&&context.window.ProjectCurseAudioManifest.events['map.brief'].bus==='interface'&&context.window.ProjectCurseAudioManifest.events['map.brief'].gain<=.42&&context.window.ProjectCurseAudioManifest.events['map.brief'].cooldown>=700&&mapRoomRuntime.includes("?'map.brief':fallback")&&mapRoomRuntime.includes('detailCueForSite(state.detailSite'));
...
add('transition-controller-owned',context.window.ProjectCurseTransitions?.screens&&Object.keys(context.window.ProjectCurseTransitions.screens).length===7&&transitionController.includes('ProjectCurseTransition')&&transitionController.includes("dataset.transitionState='switching'"));
...
add('map-region-name-authority',context.window.ProjectCurseMapRoom?.version==='map-room-v11'&&namedMapRegions.map(region=>region.id).join('|')==='northamerica|southamerica'&&namedMapRegions.every(region=>region.nomenclature?.primary&&region.nomenclature?.scope&&region.nomenclature?.boundary)&&mapRoomRuntime.includes('<dt>기록 표제</dt>')&&mapRoomRuntime.includes('<dt>지리 범위</dt>')&&mapRoomRuntime.includes('<dt>폐기 표기</dt>'));
add('map-location-deep-links',appShell.includes('mapRoomPath:')&&appShell.includes("request.route==='map-room'")&&appShell.includes('runtime.openLocation?.(request.mapRoomPath')&&mapRoomRuntime.includes('function mapLocationParts')&&mapRoomRuntime.includes('function applyMapLocationPath')&&mapRoomRuntime.includes("['detail',state.detail")&&mapRoomRuntime.includes("['region',state.region")&&mapRoomRuntime.includes("['operation',state.operation")&&mapRoomRuntime.includes('matchesRestoredLocation')&&mapRoomRuntime.includes('openLocation(path,options={})')&&mapRoomRuntime.includes('data-map-copy-link'));
add('map-recent-coordinate-history',mapRoomRuntime.includes("recentKey='project_curse_map_recent_v1'")&&mapRoomRuntime.includes('recentLimit=4')&&mapRoomRuntime.includes('function mapLocationDescriptor')&&mapRoomRuntime.includes('function rememberMapLocation')&&mapRoomRuntime.includes('data-map-recent=')&&mapRoomRuntime.includes('getRecentLocations:')&&mapRoomCss.includes('.pc-map-recent-list')&&mapRoomCss.includes('@media (max-width: 560px)'));
add('map-tab-roving-keyboard',mapRoomRuntime.includes("const tabKeys=['ArrowLeft','ArrowRight','ArrowUp','ArrowDown','Home','End']")&&mapRoomRuntime.includes('focusRenderedTab(identity)')&&count(mapRoomRuntime,'role="tab" aria-selected=')>=4&&count(mapRoomRuntime,'tabindex="${')>=4);
...
add('regional-northern-front-briefing',drilldowns.some(detail=>detail.id==='eastasia-northern-front'&&detail.terrain==='front'&&detail.sites.length===9&&detail.routes.length===3&&detail.sites.some(site=>site.history==='2038-06-29-sixth-northern-line')&&detail.visual?.src?.includes('northern-front-duplicate-signal-reconstruction')&&detail.visual?.history==='2026-08-20-northern-reversal'&&detail.signalBrief?.lanes?.length===2&&detail.signalBrief?.checks?.length===3&&detail.signalBrief?.log?.length===3)&&mapRoomRuntime.includes("target:{kind:'detail',id:'eastasia-northern-front'}")&&mapRoomRuntime.includes("target:{kind:'detail',id:'gbf-coastal-belt'}")&&mapRoomRuntime.includes("detail.terrain==='front'")&&mapRoomRuntime.includes('site.history||incident?.history')&&mapRoomRuntime.includes('renderDetailVisual')&&mapRoomRuntime.includes('renderDetailSignalBrief')&&mapRoomRuntime.includes('ProjectCurseMedia?.enhance'));
add('regional-europe-north-sea-briefing',drilldowns.some(detail=>detail.id==='europe-north-sea-blockade'&&detail.terrain==='northsea'&&detail.operation==='op-immortality'&&detail.sites.length===9&&detail.routes.length===3&&detail.sites.some(site=>site.id==='europe-blood-lake'&&site.incident==='evt-blood-lake'&&site.records?.join('|')==='Immortality_860201|Unknown_Record2_860205')&&detail.visual?.src?.includes('north-sea-blood-lake-blockade')&&detail.visual?.assetId==='VEA-NS-BL-01'&&detail.signalBrief?.title==='회수 접근 / 마지막 신호 역전')&&visualEvidence?.known?.['assets/resources/derived/north-sea-blood-lake-blockade-reconstruction-concept-v1.png']?.assetId==='VEA-NS-BL-01'&&mapRoomRuntime.includes("'blood-lake-site':'europe-north-sea-blockade'")&&mapRoomRuntime.includes("detail.terrain==='northsea'")&&mapRoomCss.includes('.pc-detail-northsea-blockade'));
...
add('regional-drilldown-navigation',mapRoomRuntime.includes("state.mode='region'")&&mapRoomRuntime.includes("state.mode==='detail'")&&mapRoomRuntime.includes('data-map-open-detail')&&mapRoomRuntime.includes('data-map-detail-site')&&mapRoomRuntime.includes('showDetail(id,siteId)'));
...
add('history-map-rendered-marker-gate',worldHistory.includes('mappedIncidentIds')&&worldHistory.includes('mappedOperationIds')&&worldHistory.includes('if(mappedIncidentIds.has(incident.id))')&&context.window.ProjectCurseIncidentNetwork?.incidents?.['evt-amarion-foundation']?.coordinates?.length===2&&!context.window.ProjectCurseMapRoom?.markers?.some(marker=>marker.incident==='evt-amarion-foundation'));
add('mobile-map-intel-panel',mapRoomRuntime.includes('function renderIntelPanel')&&mapRoomRuntime.includes('data-map-intel-toggle')&&mapRoomRuntime.includes('state.intelCollapsed')&&mapRoomCss.includes('.pc-map-intel-panel.is-collapsed .pc-map-intel-body'));
add('mobile-map-detail-directory',mapRoomRuntime.includes('function renderMobileDetailSiteIndex')&&mapRoomRuntime.includes('data-map-detail-source="list"')&&mapRoomRuntime.includes('focusDetailIntelAfterRender')&&mapRoomCss.includes('.pc-map-mobile-site-index')&&mapRoomCss.includes('min-height:68px'));
add('mobile-map-brief-entry',mapRoomRuntime.includes('function renderMobileDetailBriefEntry')&&mapRoomRuntime.includes('data-map-detail-brief')&&mapRoomRuntime.includes('RECOVERED BRIEFING')&&mapRoomCss.includes('.pc-map-mobile-brief-entry'));
add('signal-index-owner',structureData?.owners?.mapSignalIndex==='assets/js/data/map-signal-index-data.js'&&index.includes(`assets/js/data/map-signal-index-data.js?v=${VERSION}`)&&index.indexOf('map-room-data.js')<index.indexOf('map-signal-index-data.js')&&index.indexOf('map-signal-index-data.js')<index.indexOf('map-room.js'));
...
add('signal-index-canon-boundary',mapSignalIndex?.items?.filter(item=>item.mapStatus==='withheld').map(item=>item.target.id).join('|')==='evt-amarion-foundation'&&mapSignalIndex.items.filter(item=>item.category==='synchrony').every(item=>item.mapStatus==='independent'&&item.unresolved)&&mapSignalIndexSource.includes("status:'MAP POSITION WITHHELD'")&&mapRoomRuntime.includes('POSITION WITHHELD 항목은 사건을 부정하지 않는다'));
add('signal-index-search-filter-handoff',mapRoomRuntime.includes('function renderSignalIndex')&&mapRoomRuntime.includes('function updateSignalIndex')&&mapRoomRuntime.includes('function selectIndexItem')&&mapRoomRuntime.includes('data-map-index-search')&&mapRoomRuntime.includes('data-map-index-filter')&&mapRoomRuntime.includes("item.target.kind==='synchrony'")&&mapRoomRuntime.includes("item.target.kind==='operation'"));
add('signal-index-mobile-accessibility',mapRoomRuntime.includes('role="dialog"')&&mapRoomRuntime.includes('aria-modal=')&&mapRoomRuntime.includes("event.key==='Escape'")&&mapRoomRuntime.includes("event.key==='Tab'")&&mapRoomRuntime.includes("openSignalIndex(query='')")&&mapRoomCss.includes('.pc-map-signal-index.is-open')&&mapRoomCss.includes('height:min(68dvh,640px)'));
add('map-session-restore',mapRoomRuntime.includes("sessionKey='project_curse_map_session_v1'")&&mapRoomRuntime.includes('function restoreMapSession')&&mapRoomRuntime.includes('function saveMapSession')&&mapRoomRuntime.includes('detailLayers:{...state.detailLayers}')&&mapRoomRuntime.includes('intelCollapsed:state.intelCollapsed')&&mapRoomRuntime.includes('indexQuery:state.indexQuery')&&mapRoomRuntime.includes('indexFilter:state.indexFilter'));
add('map-reader-first-theater-entry',mapRoomRuntime.includes("mode:'landing'")&&mapRoomRuntime.includes('function renderTheaterIndex')&&['북부전선','대흑림','데드존','남부 쿠데타'].every(label=>mapRoomRuntime.includes(label))&&mapRoomRuntime.includes('data-map-theater=')&&mapRoomRuntime.includes('data-map-landing')&&mapRoomRuntime.includes("entryVersion:sessionVersion")&&mapRoomCss.includes('.pc-map-theater-grid')&&mapRoomCss.includes('.pc-map-workspace-head'));
...
add('archive-fourteen-record-index',publicArchiveIds.length===14&&publicArchiveIds.slice(0,4).join('|')==='Cults_871104|Immortality_860201|Ferals_860722|Zone_870815',publicArchiveIds.length);
add('archive-all-fourteen-open',archiveData?.publicRecords?.length===14&&archiveData.publicRecords.every(record=>record.access==='open'));
...
add('archive-video-document-formats',videoRecords.map(record=>record.id).join('|')==='Cults_871104|Immortality_860201|Ferals_860722|Sakuma_Tape_991028'&&documentRecords.length===10&&archiveRuntime.includes("categoryOrder=['all','video','incident','region','guide','operation','entity','cult']")&&archiveRuntime.includes('pc-archive-card-grid'),`${videoRecords.length} video / ${documentRecords.length} document`);
...
add('archive-reader-first-hierarchy',archiveRuntime.includes('const storyChapters=[')&&['금기를 사업으로 바꾸다','위협에 이름을 붙이다','기관이 서로를 배신하다','국가 이후의 생존','남부 전쟁이 모이다'].every(title=>archiveRuntime.includes(title))&&archiveRuntime.includes('function chapterMarkup')&&archiveRuntime.includes('function archiveMore')&&archiveRuntime.includes('pc-archive-tools')&&archiveRuntime.includes('pc-archive-more-stack')&&archiveRuntime.includes('data-pc-archive-chapter')&&archiveRuntime.includes('ids=cards.map(card=>card.dataset.pcArchiveOpen).sort()'));
...
add('archive-ten-internal-documents',documentRecords.length===10&&documentRecords.every(record=>!record.href)&&documentRecords.every(record=>context.window.ProjectCurseArchiveDocuments?.documents?.[record.id]));
...
add('archive-no-access-limit-label',!archiveRuntime.includes('접근 제한')&&!archiveRuntime.includes('is-restricted')&&archiveRuntime.includes("record.format==='video'?'영상 기록 재생':'문서 기록 열기'"));
add('archive-ten-readable-documents',documentRecords.length===10&&documentRecords.every(record=>context.window.ProjectCurseArchiveDocuments?.documents?.[record.id])&&read('assets/js/pages/archive-document.js').includes('ProjectCurseArchiveDocument'));
...
add('media-provenance-all-204-assets',provenanceAssets.length===204&&repositoryMedia.join('|')===provenancePaths.join('|'),`${provenanceAssets.length} registered / ${repositoryMedia.length} files`);
...
add('media-provenance-kind-counts',mediaProvenance?.stats?.byKind?.image===174&&mediaProvenance?.stats?.byKind?.audio===23&&mediaProvenance?.stats?.byKind?.video===7);
add('media-provenance-honest-review',provenanceAssets.filter(asset=>asset.kind==='audio'||asset.kind==='video').every(asset=>asset.release==='LICENSE_REVIEW')&&mediaProvenance?.stats?.review===150&&mediaProvenance?.stats?.managed===54);
...
add('media-provenance-unselected-candidates-excluded',[...unselectedCandidateMedia].every(relative=>!provenancePaths.includes(relative)&&!index.includes(relative.split('/').at(-1))));
...
add('media-provenance-delivery-lineage',provenanceAssets.filter(asset=>asset.provenance==='DELIVERY_DERIVATIVE').length===56&&provenanceAssets.filter(asset=>asset.provenance==='DELIVERY_DERIVATIVE').every(asset=>asset.derivedFrom&&provenancePaths.includes(asset.derivedFrom)));
...
add('media-provenance-public-audit-ui',archiveRuntime.includes('function provenanceAuditMarkup()')&&archiveRuntime.includes('data-pc-media-audit')&&archiveRuntime.includes('출처 검토 기록 열기')&&read('assets/css/archive-consolidation.css').includes('.pc-archive-audit-link'));
...
add('media-clearance-owner',structureData?.owners?.mediaClearanceRuntime==='assets/js/pages/media-clearance.js'&&structureData?.owners?.mediaClearanceCSS==='assets/css/media-clearance.css'&&index.includes(`id="media-audit"`)&&index.includes(`assets/js/pages/media-clearance.js?v=${VERSION}`));
add('media-clearance-search-filter-detail',mediaClearanceRuntime.includes('function visibleItems()')&&mediaClearanceRuntime.includes('data-media-search')&&mediaClearanceRuntime.includes('data-media-release')&&mediaClearanceRuntime.includes('data-media-kind')&&mediaClearanceRuntime.includes('function detailMarkup(item)'));
add('media-clearance-reference-boundary',mediaClearanceRuntime.includes('공개 자산이 아닙니다')&&mediaClearanceRuntime.includes('미디어 미리보기·자동 재생 없음')&&!mediaClearanceRuntime.includes('new Audio(')&&!mediaClearanceRuntime.includes('<img'));
add('media-clearance-responsive-accessible',mediaClearanceCss.includes('@media(max-width:820px)')&&mediaClearanceCss.includes('@media(prefers-reduced-motion:reduce)')&&mediaClearanceRuntime.includes("event.key==='ArrowDown'")&&mediaClearanceRuntime.includes("event.key==='Escape'"));
...
add('adaptive-media-twenty-eight-sources',mediaManifest?.version==='1.0.0'&&responsiveAssets.length===28&&responsiveVariants.length===56,`${responsiveAssets.length} sources / ${responsiveVariants.length} variants`);
...
add('personnel-independent-boundary',!personnelData.byId?.frey&&!/프레이|frey/i.test(index+personnelProfileSource+personnelRemakeSource+personnelDataSource+personnelRuntime)&&personnelData?.editorialRule?.includes('개편 정본명')&&personnelRuntime.includes('2006년까지 확인된 인물'));
...
add('personnel-route-and-runtime',index.includes('id="personnel"')&&count(index,'data-uac-route="personnel"')>=3&&index.includes(`assets/js/pages/personnel-archive.js?v=${VERSION}`)&&personnelRuntime.includes('ProjectCursePersonnelRuntime=Object.freeze')&&personnelRuntime.includes('data-pc-person-search'));
add('personnel-2006-reader-display',personnelRuntime.includes('PERSONNEL FILES / 2006')&&personnelRuntime.includes("active:'2006년 활동 확인'")&&personnelRuntime.includes("deceased:'2006년 사망 기재'")&&personnelRuntime.includes("unknown:'2006년 이후 미확인'")&&!personnelRuntime.includes('구 명부명')&&!personnelRuntime.includes('호칭·별칭')&&!personnelRuntime.includes('자료 상태'));
add('personnel-summary-before-selection',personnelRuntime.includes('function summaryDetailMarkup')&&personnelRuntime.includes('인물을 선택하십시오')&&personnelRuntime.includes('과거와 관계는 필요한 경우에만')&&personnelCss.includes('.pc-personnel-summary'));
add('personnel-direct-hash-navigation',appShell.includes('personnelId:route===\'personnel\'')&&appShell.includes('applyLocationDetail')&&personnelRuntime.includes("return id?`#personnel/${encodeURIComponent(id)}`:'#personnel'")&&personnelRuntime.includes("historyMode='push'")&&personnelRuntime.includes('data-pc-person-copy-link'));
add('personnel-mobile-dossier-navigation',personnelRuntime.includes('data-pc-person-back')&&personnelRuntime.includes('data-pc-person-prev')&&personnelRuntime.includes('data-pc-person-next')&&personnelCss.includes('.pc-personnel-has-selection .pc-personnel-index{display:none}')&&personnelCss.includes('.pc-personnel-detail-nav{position:sticky'));
add('personnel-reader-first-presentation',personnelRuntime.includes('function identityMarkup')&&personnelRuntime.includes('function backgroundMarkup')&&personnelRuntime.includes('class="pc-personnel-more"')&&personnelRuntime.includes('신원과 과거')&&personnelRuntime.includes('관계와 추가 기록')&&personnelRuntime.includes('record.sourceName')&&personnelCss.includes('.pc-personnel-more'));
...
add('history-limit-and-map-link-runtime',worldHistory.includes('data-history-record-limit')&&worldHistory.includes('이 기록으로 확정할 수 없는 것')&&worldHistory.includes('관측 좌표')&&worldHistory.includes('작전 레이어')&&worldHistoryCss.includes('.pc-world-history-limit'));
add('history-reader-first-hierarchy',worldHistory.includes('data-history-reader-record')&&worldHistory.includes('먼저 볼 네 전환점')&&worldHistory.includes('pc-world-history-reference-stack')&&worldHistory.includes('pc-world-history-record-context')&&worldHistory.includes("document.createElement('details')")&&worldHistoryCss.includes('.pc-world-history-turns')&&worldHistoryCss.includes('.pc-world-history-record-context'));
...
add('history-visual-responsive-runtime',worldHistory.includes('image.dataset.pcSource=record.visual.src')&&worldHistory.includes('window.ProjectCurseMedia.apply')&&!worldHistory.includes('root.ProjectCurseMedia')&&worldHistory.includes("sizes:'(max-width: 760px) 94vw, 960px'"));
...
add('fourteen-concept-images-selectively-integrated',publicConceptAssets.every(name=>publicConceptSources.includes(name))&&!publicConceptSources.includes('project-curse-world-keyart-concept-v2.png'));
