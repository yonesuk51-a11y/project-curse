...
add('archive-video-document-formats',videoRecords.map(record=>record.id).join('|')==='Cults_871104|Immortality_860201|Ferals_860722|Sakuma_Tape_991028'&&documentRecords.length===11&&archiveRuntime.includes("categoryOrder=['all','video','incident','region','guide','operation','entity','cult']")&&archiveRuntime.includes('pc-archive-card-grid'),`${videoRecords.length} video / ${documentRecords.length} document`);
...
add('archive-eleven-readable-documents',documentRecords.length===11&&documentRecords.every(record=>context.window.ProjectCurseArchiveDocuments?.documents?.[record.id])&&read('assets/js/pages/archive-document.js').includes('ProjectCurseArchiveDocument'));
...
add('media-provenance-all-219-assets',provenanceAssets.length===219&&repositoryMedia.join('|')===provenancePaths.join('|'),`${provenanceAssets.length} registered / ${repositoryMedia.length} files`);
...
add('media-provenance-kind-counts',mediaProvenance?.stats?.byKind?.image===189&&mediaProvenance?.stats?.byKind?.audio===23&&mediaProvenance?.stats?.byKind?.video===7);
add('media-provenance-honest-review',provenanceAssets.filter(asset=>asset.kind==='audio'||asset.kind==='video').every(asset=>asset.release==='LICENSE_REVIEW')&&mediaProvenance?.stats?.review===150&&mediaProvenance?.stats?.managed===69);
...
add('media-provenance-delivery-lineage',provenanceAssets.filter(asset=>asset.provenance==='DELIVERY_DERIVATIVE').length===66&&provenanceAssets.filter(asset=>asset.provenance==='DELIVERY_DERIVATIVE').every(asset=>asset.derivedFrom&&provenancePaths.includes(asset.derivedFrom)));
...
add('adaptive-media-thirty-three-sources',mediaManifest?.version==='1.0.0'&&responsiveAssets.length===33&&responsiveVariants.length===66,`${responsiveAssets.length} sources / ${responsiveVariants.length} variants`);
...
add('faction-seventeen-context-assessments',factionAnalysis?.order?.length===17&&factionAnalysis.order.every(key=>factionAssessmentFields.every(field=>factionAnalysis.factions[key]?.assessment?.[field]))&&factionAnalysisRuntime.includes('function factionAssessment')&&factionAnalysisRuntime.includes('주의할 점'));
