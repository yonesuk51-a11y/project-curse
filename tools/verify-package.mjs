...
add('shell-quick-nav-six-lore-one-utility',(index.match(/<a[^>]+data-uac-route="(?:map-room|history|faction-info|archive-entry|media-audit|personnel)"[^>]*>/g)||[]).length>=6&&index.includes('class="pc-shell-utility-route" data-uac-route="media-audit"')&&channelIdentityRuntime.includes("channel.navTier==='utility'")&&appShell.includes('closeQuickNav'));
...
add('home-utility-register-separated',index.includes('class="pc-terminal-utilities"')&&index.includes('세계관 바깥의 열람 도구')&&index.includes('data-pc-preferences-open')&&count(index,'<b>미디어 감사</b>')===1);
