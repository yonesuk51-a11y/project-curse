...
add('channel-identity-five-public-channels',channelIdentityData?.channels?.length===5&&channelIdentityData.channels.map(channel=>channel.id).join('|')==='terminal-home|map-room|history|faction-info|archive-entry');
...
add('channel-minimal-audio-default',channelIdentityData?.defaults?.interfaceAudio==='minimal'&&channelIdentityData?.defaults?.ambient==='off'&&audioController.includes('ambient:0,interface:.34,record:.58'));
...
add('telemetry-public-and-operator-channels',['terminal-home','map-room','history','faction-info','archive-entry','media-audit'].every(id=>performanceTelemetry.includes(`id==='${id}'`))&&channelIdentityRuntime.includes('ProjectCurseTelemetry?.getChannelStatus'));
...
add('shell-quick-nav-four-public-links',(index.match(/<a[^>]+data-uac-route="(?:map-room|history|faction-info|archive-entry)"[^>]*>/g)||[]).length>=4&&!index.includes('data-uac-route="media-audit"')&&appShell.includes('closeQuickNav'));
add('home-lore-entry-routes',index.includes('class="pc-terminal-entry-routes"')&&index.includes('세계를 이해한다')&&index.includes('대표 기록을 본다')&&index.includes('전장에 진입한다')&&index.includes('class="pc-terminal-alert"')&&index.includes('class="pc-terminal-recent"'));
...
add('current-five-screen-public-manifest',structureData?.screens?.map(screen=>screen.id).join('|')==='terminal-home|map-room|history|faction-info|archive-entry');
add('operator-media-screen-boundary',structureData?.internalScreens?.map(screen=>screen.id).join('|')==='media-audit'&&appShell.includes("get('operator')==='media'")&&mediaClearanceRuntime.includes('operatorAccess')&&mediaClearanceRuntime.includes('available:false'));
...
add('loading-modes-and-readable-timing',loadingRuntime.includes('duration:3600')&&loadingRuntime.includes('finishDelay:260')&&loadingRuntime.includes('duration:700')&&loadingRuntime.includes('finishDelay:80')&&loadingRuntime.includes('duration:420')&&loadingRuntime.includes('finishDelay:40')&&loadingRuntime.includes('duration:280')&&loadingRuntime.includes('MIN_VISIBLE_MS=0')&&loadingRuntime.includes('FINAL ACCESS HOLD')&&loadingRuntime.includes("classList.add('is-authorized')")&&loadingRuntime.includes('SESSION_KEY=()=>')&&loadingRuntime.includes("get('boot')")&&index.includes('data-boot-gate'));
...
add('acoustic-screen-profiles',['terminal-home','map-room','history','faction-info','archive-entry','media-audit','document','great-black-forest','dead-zone','guide','scenario','recovery-scenario'].every(profile=>context.window.ProjectCurseAudioManifest?.profiles?.[profile]));
...
add('transition-controller-owned',context.window.ProjectCurseTransitions?.screens&&Object.keys(context.window.ProjectCurseTransitions.screens).length===6&&transitionController.includes('ProjectCurseTransition')&&transitionController.includes("dataset.transitionState='switching'"));
...
add('media-provenance-public-credit-note',archiveRuntime.includes('function provenanceNoticeMarkup()')&&archiveRuntime.includes('pc-public-credits-note')&&archiveRuntime.includes('MEDIA_CREDITS.md')&&read('assets/css/archive-consolidation.css').includes('.pc-public-credits-note'));
...
add('faction-single-menu',count(index,'data-uac-route="faction-info"')>=2&&count(index,'data-uac-route="faction-relation"')===0&&index.includes('<b>세력 정보</b>'));
