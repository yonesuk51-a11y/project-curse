...
add('regional-northern-front-briefing',drilldowns.some(detail=>detail.id==='eastasia-northern-front'&&detail.terrain==='front'&&detail.sites.length===9&&detail.routes.length===3&&detail.sites.some(site=>site.history==='2038-06-29-sixth-northern-line'))&&mapRoomRuntime.includes("target:{kind:'detail',id:'eastasia-northern-front'}")&&mapRoomRuntime.includes("target:{kind:'detail',id:'gbf-coastal-belt'}")&&mapRoomRuntime.includes("detail.terrain==='front'")&&mapRoomRuntime.includes('site.history||incident?.history'));
...
add('media-provenance-all-184-assets',provenanceAssets.length===184&&repositoryMedia.join('|')===provenancePaths.join('|'),`${provenanceAssets.length} registered / ${repositoryMedia.length} files`);
...
add('media-provenance-kind-counts',mediaProvenance?.stats?.byKind?.image===154&&mediaProvenance?.stats?.byKind?.audio===23&&mediaProvenance?.stats?.byKind?.video===7);
add('media-provenance-honest-review',provenanceAssets.filter(asset=>asset.kind==='audio'||asset.kind==='video').every(asset=>asset.release==='LICENSE_REVIEW')&&mediaProvenance?.stats?.review===150&&mediaProvenance?.stats?.managed===34);
