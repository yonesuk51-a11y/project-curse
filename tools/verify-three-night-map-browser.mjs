...
async function openMap(viewport,label){
  ...
  check(`${label}:build`,await page.evaluate(()=>window.ProjectCurseBuild?.version)==='5.45.0');
...
const gbfIntel=await desktop.page.evaluate(()=>(
  {
    ...
    lead:document.querySelector('.pc-map-intel>p')?.textContent.replace(/\s+/g,' ').trim(),
...
const desktopShot=join(tmpdir(),'project-curse-5.45.0-synchrony-map-desktop.png');
...
const mobileShot=join(tmpdir(),'project-curse-5.45.0-synchrony-map-mobile.png');
