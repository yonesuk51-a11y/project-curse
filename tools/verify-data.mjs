...
function verify(){
  ...
  add('media-provenance-all-522-assets',provenanceAssets.length===522&&repositoryMedia.join('|')===provenancePaths.join('|'),`${provenanceAssets.length} registered / ${repositoryMedia.length} files`);
  ...
  add('media-provenance-kind-counts',mediaProvenance?.stats?.byKind?.image===492&&mediaProvenance?.stats?.byKind?.audio===23&&mediaProvenance?.stats?.byKind?.video===7);
  add('media-provenance-honest-review',provenanceAssets.filter(asset=>asset.kind==='audio'||asset.kind==='video').every(asset=>asset.release==='LICENSE_REVIEW')&&mediaProvenance?.stats?.review===150&&mediaProvenance?.stats?.managed===372);
  ...
  add('media-provenance-delivery-lineage',provenanceAssets.filter(asset=>asset.provenance==='DELIVERY_DERIVATIVE').length===252&&provenanceAssets.filter(asset=>asset.provenance==='DELIVERY_DERIVATIVE').every(asset=>asset.derivedFrom&&provenancePaths.includes(asset.derivedFrom)));
  ...
  add('adaptive-media-one-hundred-twenty-six-sources',mediaManifest?.version==='1.0.0'&&responsiveAssets.length===126&&responsiveVariants.length===252,`${responsiveAssets.length} sources / ${responsiveVariants.length} variants`);
