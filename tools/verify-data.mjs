...
function verify(){
  ...
  add('media-provenance-all-546-assets',provenanceAssets.length===546&&repositoryMedia.join('|')===provenancePaths.join('|'),`${provenanceAssets.length} registered / ${repositoryMedia.length} files`);
  ...
  add('media-provenance-kind-counts',mediaProvenance?.stats?.byKind?.image===516&&mediaProvenance?.stats?.byKind?.audio===23&&mediaProvenance?.stats?.byKind?.video===7);
  add('media-provenance-honest-review',provenanceAssets.filter(asset=>asset.kind==='audio'||asset.kind==='video').every(asset=>asset.release==='LICENSE_REVIEW')&&mediaProvenance?.stats?.review===150&&mediaProvenance?.stats?.managed===396);
  ...
  add('media-provenance-delivery-lineage',provenanceAssets.filter(asset=>asset.provenance==='DELIVERY_DERIVATIVE').length===268&&provenanceAssets.filter(asset=>asset.provenance==='DELIVERY_DERIVATIVE').every(asset=>asset.derivedFrom&&provenancePaths.includes(asset.derivedFrom)));
  ...
  add('adaptive-media-one-hundred-thirty-four-sources',mediaManifest?.version==='1.0.0'&&responsiveAssets.length===134&&responsiveVariants.length===268,`${responsiveAssets.length} sources / ${responsiveVariants.length} variants`);
  ...
  add('archive-nhc-media-links',nhcManualMedia.length===6&&nhcManualMedia.every(media=>existsSync(path(media.src.replace(/^\.\.\/\.\.\//,'')))),nhcManualMedia.length);
  ...
  add('personnel-profile-schema',personnelProfiles?.version===VERSION&&personnelProfiles?.schema==='project-curse-personnel-profile-v1'&&personnelProfiles?.status==='SUPPLEMENTAL IDENTITY / PROVISIONAL'&&Object.keys(personnelProfiles.profiles||{}).length===21);
  const personnelRemake=context.window.ProjectCursePersonnelRemake;
  add('personnel-remake-schema',personnelRemake?.version===VERSION&&personnelRemake?.schema==='project-curse-personnel-remake-v1'&&Object.keys(personnelRemake.records||{}).length===21&&Object.keys(personnelRemake.groupOverrides||{}).length===8);
  add('personnel-registry-schema',personnelData?.version===VERSION&&personnelData?.schema==='project-curse-personnel-v3'&&personnelData.records.length===21&&personnelData.groups.length===8,`${personnelData?.records?.length||0} records / ${personnelData?.groups?.length||0} groups`);
  add('personnel-registry-unique-ids',new Set(personnelData?.records?.map(record=>record.id)).size===21);
  ...
  add('personnel-complete-supplemental-dossiers',personnelData?.stats?.profiled===21&&personnelData.records.every(record=>record.identity?.sex&&record.identity?.birth&&record.identity?.age&&record.identity?.origin&&record.identity?.nationality&&record.affiliationSummary&&record.personality?.temperament&&record.personality?.drive&&record.personality?.fear&&record.background?.length>=2&&record.history?.length>=3&&record.fieldNotes?.length),personnelData?.stats?.profiled||0);
  ...
  add('personnel-renamed-alias-preservation',personnelData?.stats?.renamed>=20&&personnelData.records.filter(record=>record.sourceName).every(record=>record.aliases?.includes(record.sourceName))&&personnelData.byId?.baranto?.name==='마테오 오르테가'&&personnelData.byId?.baranto?.aliases?.includes('바란토'));
  ...
  add('personnel-legacy-status-boundary',personnelData?.stats?.deceased===1&&personnelData.records.filter(record=>record.status==='deceased').map(record=>record.id).join('|')==='reiki');
  ...
  add('personnel-faction-index',personnelData?.factionIndex?.fhc?.length===6&&personnelData?.factionIndex?.ushinoda?.length===4&&personnelData?.factionIndex?.['blood-cult']?.length===1&&personnelData?.factionIndex?.['corruption-cult']?.length===1&&personnelData?.factionIndex?.nhc?.length===3);
  ...
  // 2026-09-25 사용자 설정: 리버스 지점의 피탈자. 네 구분에 끼워 넣지 않고, 분류 근거·교범 교전 규칙·해제 조건·의식과 생사를 함께 둔다.
  const possession=worldHistoryData?.worldFramework?.reverseSiteCivilians;
  add('history-reverse-site-possession:data',Boolean(possession?.label&&possession.rule&&possession.handling&&possession.caution)&&possession.record==='Ferals_860722'&&possession.rule.includes('몸을 빼앗는 힘은 리버스다')&&possession.rule.includes('피탈자')&&Boolean(possession.state&&possession.release&&possession.afterDeath)&&possession.afterDeath.includes('핏덩이')&&Boolean(context.window.ProjectCurseArchiveDocuments?.documents?.NHC_Manual_891219?.sections?.some(s=>s.title===possession.manualSection&&s.record?.limit&&(s.items||[]).length>=5))&&Boolean(context.window.ProjectCurseArchiveDocuments?.documents?.Ferals_860722?.sections?.some(s=>(s.items||[]).some(t=>t.includes('빙의 상태의 생존자를 자동으로 괴이 분류에 넣지 않는다')))));
