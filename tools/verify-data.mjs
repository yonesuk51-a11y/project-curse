...
function verify(){
  ...
  add('personnel-profile-schema',personnelProfiles?.version===VERSION&&personnelProfiles?.schema==='project-curse-personnel-profile-v1'&&personnelProfiles?.status==='SUPPLEMENTAL IDENTITY / PROVISIONAL'&&Object.keys(personnelProfiles.profiles||{}).length===20);
  const personnelRemake=context.window.ProjectCursePersonnelRemake;
  add('personnel-remake-schema',personnelRemake?.version===VERSION&&personnelRemake?.schema==='project-curse-personnel-remake-v1'&&Object.keys(personnelRemake.records||{}).length===20&&Object.keys(personnelRemake.groupOverrides||{}).length===8);
  add('personnel-registry-schema',personnelData?.version===VERSION&&personnelData?.schema==='project-curse-personnel-v3'&&personnelData.records.length===20&&personnelData.groups.length===8,`${personnelData?.records?.length||0} records / ${personnelData?.groups?.length||0} groups`);
  add('personnel-registry-unique-ids',new Set(personnelData?.records?.map(record=>record.id)).size===20);
  ...
  add('personnel-complete-supplemental-dossiers',personnelData?.stats?.profiled===20&&personnelData.records.every(record=>record.identity?.sex&&record.identity?.birth&&record.identity?.age&&record.identity?.origin&&record.identity?.nationality&&record.affiliationSummary&&record.personality?.temperament&&record.personality?.drive&&record.personality?.fear&&record.background?.length>=2&&record.history?.length>=3&&record.fieldNotes?.length),personnelData?.stats?.profiled||0);
  ...
  add('personnel-renamed-alias-preservation',personnelData?.stats?.renamed>=19&&personnelData.records.filter(record=>record.sourceName).every(record=>record.aliases?.includes(record.sourceName))&&personnelData.byId?.baranto?.name==='마테오 오르테가'&&personnelData.byId?.baranto?.aliases?.includes('바란토'));
  ...
  add('personnel-faction-index',personnelData?.factionIndex?.fhc?.length===6&&personnelData?.factionIndex?.ushinoda?.length===3&&personnelData?.factionIndex?.['blood-cult']?.length===1&&(personnelData?.factionIndex?.['corruption-cult']||[]).length===0&&personnelData?.factionIndex?.nhc?.length===3);
