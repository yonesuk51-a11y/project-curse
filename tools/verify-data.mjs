...
function verify(){
  ...
  add('media-provenance-all-356-assets',provenanceAssets.length===356&&repositoryMedia.join('|')===provenancePaths.join('|'),`${provenanceAssets.length} registered / ${repositoryMedia.length} files`);
  ...
  add('media-provenance-kind-counts',mediaProvenance?.stats?.byKind?.image===326&&mediaProvenance?.stats?.byKind?.audio===23&&mediaProvenance?.stats?.byKind?.video===7);
  add('media-provenance-honest-review',provenanceAssets.filter(asset=>asset.kind==='audio'||asset.kind==='video').every(asset=>asset.release==='LICENSE_REVIEW')&&mediaProvenance?.stats?.review===150&&mediaProvenance?.stats?.managed===206);
  ...
  add('media-provenance-delivery-lineage',provenanceAssets.filter(asset=>asset.provenance==='DELIVERY_DERIVATIVE').length===146&&provenanceAssets.filter(asset=>asset.provenance==='DELIVERY_DERIVATIVE').every(asset=>asset.derivedFrom&&provenancePaths.includes(asset.derivedFrom)));
  ...
  add('adaptive-media-seventy-three-sources',mediaManifest?.version==='1.0.0'&&responsiveAssets.length===73&&responsiveVariants.length===146,`${responsiveAssets.length} sources / ${responsiveVariants.length} variants`);
