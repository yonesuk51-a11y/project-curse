...
async function openSite(viewport,label){
  ...
  await page.waitForSelector('#app.ready',{timeout:12000});
  check(`${label}:build`,await page.evaluate(()=>window.ProjectCurseBuild?.version)==='5.46.0');
...
const desktopShot=join(tmpdir(),'project-curse-5.46.0-history-map-density-desktop.png');
...
const mobileShot=join(tmpdir(),'project-curse-5.46.0-history-map-density-mobile.png');
