...
check('desktop:data-ledger',dataState.registered===184&&dataState.review===150&&dataState.managed===34,JSON.stringify(dataState));
...
check('desktop:all-assets',await desktop.page.locator('[data-media-path]').count()===184);
await desktop.page.locator('[data-media-kind="image"]').click();
check('desktop:image-filter',await desktop.page.locator('[data-media-path]').count()===154);
...
check('desktop:managed-filter',await desktop.page.locator('[data-media-path]').count()===34);
...
const desktopShot=join(tmpdir(),'project-curse-5.53.0-media-clearance-desktop.png');
...
const mobileTopShot=join(tmpdir(),'project-curse-5.53.0-media-clearance-mobile-top.png');
...
const mobileShot=join(tmpdir(),'project-curse-5.53.0-media-clearance-mobile.png');
