...
check('desktop:data-ledger',dataState.registered===186&&dataState.review===150&&dataState.managed===36,JSON.stringify(dataState));
...
check('desktop:all-assets',await desktop.page.locator('[data-media-path]').count()===186);
await desktop.page.locator('[data-media-kind="image"]').click();
check('desktop:image-filter',await desktop.page.locator('[data-media-path]').count()===156);
...
check('desktop:managed-filter',await desktop.page.locator('[data-media-path]').count()===36);
...
const desktopShot=join(tmpdir(),'project-curse-5.54.0-media-clearance-desktop.png');
...
const mobileTopShot=join(tmpdir(),'project-curse-5.54.0-media-clearance-mobile-top.png');
...
const mobileShot=join(tmpdir(),'project-curse-5.54.0-media-clearance-mobile.png');
