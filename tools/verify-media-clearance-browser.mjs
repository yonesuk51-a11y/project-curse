...
async function openAudit(viewport,label){
  ...
  requests.length=0;
  check(`${label}:build`,await page.evaluate(()=>window.ProjectCurseBuild?.version)==='5.48.2');
...
const desktopShot=join(tmpdir(),'project-curse-5.48.2-media-clearance-desktop.png');
...
const mobileTopShot=join(tmpdir(),'project-curse-5.48.2-media-clearance-mobile-top.png');
...
const mobileShot=join(tmpdir(),'project-curse-5.48.2-media-clearance-mobile.png');
