...
async function openAudit(viewport,label){
  ...
  requests.length=0;
  check(`${label}:build`,await page.evaluate(()=>window.ProjectCurseBuild?.version)==='5.52.0');
...
const desktopShot=join(tmpdir(),'project-curse-5.52.0-media-clearance-desktop.png');
...
const mobileTopShot=join(tmpdir(),'project-curse-5.52.0-media-clearance-mobile-top.png');
...
const mobileShot=join(tmpdir(),'project-curse-5.52.0-media-clearance-mobile.png');
