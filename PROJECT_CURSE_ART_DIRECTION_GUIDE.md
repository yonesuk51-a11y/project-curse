...
- 채널 인계: 상단 바의 채널 표지 자리에 요청 › 단계 › 상태가 지나가고 바 아래 1px 선이 그어진다(약 0.9초). 본문을 가리지 않는다.
- 단말 기동: 세션마다 한 번, 주소 없이 홈으로 들어왔을 때만 기동 대조 다섯 줄(약 1.2초). 공유 링크로 바로 들어오면 없다. 아무 키로 건너뛴다.
- 증거 현상: 증거 사진이 어두운 필름에서 떠오른다. 가림 막대에는 낮은 잡음. 증거 층에만.
- 이상: 홈 접촉 보고의 열원이 육안 인원과 같게 잡혔다가 다시 세면 하나 더 나온다(세션마다 한 번). 이상 연출은 드물게, 최종 값은 처음부터 화면 낭독기에 준다.
- 모든 연출은 조작을 막지 않고, 모션 감소 설정이면 없다. 번쩍임·흔들림·글리치 남용을 쓰지 않는다.
