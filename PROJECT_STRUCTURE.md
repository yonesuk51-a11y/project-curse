# Project Curse Structure — 5.15.2co
...
화면 전환 시 선택 화면의 `inert`, `aria-hidden`, `pointer-events`를 같은 이벤트 안에서 해제한다. 메뉴 서랍은 PC·모바일 모두 닫힌 상태로 돌아가며 버튼, 배경막과 저장 상태도 함께 `☰ / closed`로 동기화한다.
...
- `assets/js/core/runtime-ownership.js`: 실사용 메뉴 요소에서 구형 직접 이벤트를 제거하고 중복 ID·활성 화면을 감시한다.
- `assets/js/core/menu-audio-runtime.js`: 화면 이동, 서랍 열기·닫기, 버튼 `☰/×`, 배경막, 메뉴 음향 상태를 단독 관리한다.
- `assets/js/main.js`: 보호된 기록 연출과 이전 문서 호환을 위한 호환 런타임으로만 유지한다.
