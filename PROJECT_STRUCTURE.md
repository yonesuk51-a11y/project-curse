# Project Curse Structure — 5.51.0
...
| 인물 검색·필터·상세 기록·세력 교차 이동 | `assets/js/pages/personnel-archive.js` + `assets/css/personnel-archive.css` |
...
| 일곱 채널 명칭·색상·계기·표식·표시 및 자동 압축 정책 | `assets/js/data/channel-identity-data.js` |
...
활성 화면은 `terminal-home`, `map-room`, `history`, `faction-info`, `archive-entry`, `media-audit`, `personnel` 일곱 개다. 폐기된 별도 지도 주소 `region-map`, `zone-map`, `operation-map`은 통합 관제도로 전환하고 `faction-relation`은 정보 분석으로 전환한다.
